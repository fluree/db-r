...
///
/// When `signing` is `Some((key, ledger_address))`, the commit is signed with
...
pub fn write_commit(
    ...
    if let Some((signing_key, ledger_address)) = signing {
        let _span = tracing::debug_span!("v2_write_sign").entered();
        let signature = sign_commit_digest(signing_key, &hash_bytes, ledger_address);
