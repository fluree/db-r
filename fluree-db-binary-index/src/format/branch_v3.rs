...
//!
//! Same role as FBR2 but uses V2 record keys (30 bytes) and carries
...
impl BranchManifestV3 {
    /// Binary search for the leaf containing the given key using the given order.
    pub fn find_leaf(&self, key: &RunRecordV2, order: RunSortOrder) -> Option<&LeafEntryV3> {
...
pub fn build_branch_v3_bytes(order: RunSortOrder, g_id: u16, leaves: &[LeafEntryV3]) -> Vec<u8> {
    let estimated =
...
pub fn read_branch_v3_from_bytes(data: &[u8]) -> io::Result<BranchManifestV3> {
    ...
    Ok(BranchManifestV3 { leaves })
}

/// Read the sort order from an FBR3 header without full decode.
pub fn read_branch_v3_order(data: &[u8]) -> io::Result<RunSortOrder> {
    if data.len() < BRANCH_V3_HEADER_LEN {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
...
pub fn read_branch_v3_g_id(data: &[u8]) -> io::Result<u16> {
    if data.len() < BRANCH_V3_HEADER_LEN {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "FBR3: too small for header",
...
mod tests {
    ...
    fn round_trip_no_sidecar() {
        ...
        let bytes = build_branch_v3_bytes(RunSortOrder::Spot, 0, &leaves);
        let manifest = read_branch_v3_from_bytes(&bytes).unwrap();
    ...
    fn round_trip_with_sidecar() {
        ...
        let bytes = build_branch_v3_bytes(RunSortOrder::Post, 2, &leaves);
        let manifest = read_branch_v3_from_bytes(&bytes).unwrap();
    ...
    #[test]
    fn header_metadata() {
        let bytes = build_branch_v3_bytes(RunSortOrder::Opst, 42, &[]);
        assert_eq!(&bytes[0..4], b"FBR3");
        assert_eq!(bytes[4], 1);
        assert_eq!(read_branch_v3_order(&bytes).unwrap(), RunSortOrder::Opst);
        assert_eq!(read_branch_v3_g_id(&bytes).unwrap(), 42);
    }
    ...
    fn deterministic() {
        ...
        let b1 = build_branch_v3_bytes(RunSortOrder::Post, 0, &leaves);
        let b2 = build_branch_v3_bytes(RunSortOrder::Post, 0, &leaves);
        assert_eq!(b1, b2);
    }
}
