...
///
/// Matches V5's `InlineOrderRouting` pattern but with V3 key types (`LeafEntryV3`).
/// Avoids a CAS round-trip for the default graph at load time.
#[derive(Debug, Clone)]
pub struct DefaultGraphOrderV3 {
...
pub struct IndexRootV6 {
    ...
    /// Same pattern as V5's `InlineOrderRouting` but with V3 key types.
    pub default_graph_orders: Vec<DefaultGraphOrderV3>,
...
impl IndexRootV6 {
    ...
    pub fn decode(data: &[u8]) -> io::Result<Self> {
        ...
        for _ in 0..order_count {
            ...
            default_graph_orders.push(DefaultGraphOrderV3 { order, leaves });
        }
        ...
        for _ in 0..named_count {
            ...
            named_graphs.push(NamedGraphRoutingV3 { g_id, orders });
        }
...
// ============================================================================
// Shared arena encode/decode (extracted from V5 for reuse)
// ============================================================================

fn write_graph_arenas_v5(buf: &mut Vec<u8>, ga: &GraphArenaRefsV5) {
    // numbig
...
fn read_graph_arenas_v5(data: &[u8], pos: &mut usize) -> io::Result<GraphArenaRefsV5> {
...
mod tests {
    ...
    fn minimal_root_v6() -> IndexRootV6 {
        ...
        IndexRootV6 {
            ...
            total_retracts: 0,
            graph_arenas: vec![],
            o_type_table: IndexRootV6::build_o_type_table(&[], &["en".to_string()]),
    ...
    fn fir6_round_trip_minimal() {
        ...
        let decoded = IndexRootV6::decode(&bytes).unwrap();
    ...
    fn fir6_bad_magic_rejected() {
        ...
        let err = IndexRootV6::decode(&bytes).unwrap_err();
