...
use super::leaflet_v3::{encode_leaflet_v3, EncodedLeafletV3};
...
pub struct LeafWriterV3 {
    ...
    encoded_leaflets: Vec<EncodedLeafletV3>,
    ...
    // Completed leaves.
    completed_leaves: Vec<LeafInfoV3>,
}
...
impl LeafWriterV3 {
    ...
    /// produced leaves.
    pub fn finish(mut self) -> io::Result<Vec<LeafInfoV3>> {
    ...
    fn flush_leaf(&mut self) -> io::Result<()> {
        ...
        self.completed_leaves.push(LeafInfoV3 {
            leaf_cid,
            leaf_bytes,
            sidecar_cid,
            sidecar_bytes,
...
fn compute_directory_size(
    ...
    let _ = history_seg_refs; // used for sizing, but fixed per entry
    leaflets
        .iter()
        .map(|l| {
...
mod tests {
    ...
    fn opst_segmentation_splits_on_o_type_change() {
        let mut writer = LeafWriterV3::new(RunSortOrder::Opst, 100, 1000, 1);
        writer.set_skip_history(true);
    ...
    fn leaf_splits_on_target_rows() {
        ...
        let mut writer = LeafWriterV3::new(RunSortOrder::Post, 5, 10, 1);
        writer.set_skip_history(true);
