...
impl BinaryIndexStore {
    ...
    pub fn load(run_dir: &Path, index_dir: &Path) -> io::Result<Self> {
        ...
        let mut numbig_forward: HashMap<u32, crate::arena::numbig::NumBigArena> = HashMap::new();
                ...
                if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                    if name.starts_with("p_") && name.ends_with(".nba") {
                        ...
                        if let Ok(p_id) = p_id_str.parse::<u32>() {
                            let arena = crate::arena::numbig::read_numbig_arena(&path)?;
        ...
        let mut vector_forward: HashMap<u32, crate::arena::vector::LazyVectorArena> =
            HashMap::new();
        // Leaflet cache is created below; we build arenas after that.
        // Collect manifest + shard sources first.
        struct PendingArena {
            p_id: u32,
            manifest: crate::arena::vector::VectorManifest,
            shard_sources: Vec<crate::arena::vector::ShardSource>,
        }
                ...
                if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                    if name.starts_with("p_") && name.ends_with(".vam") {
                        ...
                        if let Ok(p_id) = p_id_str.parse::<u32>() {
                            ...
                            let manifest =
                                crate::arena::vector::read_vector_manifest(&manifest_bytes)?;
    ...
    pub async fn load_from_root_bytes(
        ...
        let v5 = crate::format::index_root::IndexRootV5::decode(bytes).map_err(|e| {
            io::Error::new(
    ...
    pub async fn load_from_root_v5(
        ...
        let mut per_graph_numbig: HashMap<
            GraphId,
            HashMap<u32, crate::arena::numbig::NumBigArena>,
        > = HashMap::new();
        let mut per_graph_vectors: HashMap<
            GraphId,
            HashMap<u32, crate::arena::vector::LazyVectorArena>,
        > = HashMap::new();
    ...
    pub fn ensure_index_leaf_cached(
        ...
        cache_bytes_to_path_atomic(&leaf_path, &bytes)?;
        Ok(())
    }
    ...
    pub fn translate_object_bounds(
        ...
        _p_id: u32,
        shape: crate::types::NumericShape,
    ) -> Option<(ObjKind, ObjKey, ObjKind, ObjKey)> {
        use crate::types::NumericShape;
        match shape {
    ...
    fn numbig_arena_for_graph(
        ...
        g_id: GraphId,
        p_id: u32,
    ) -> Option<&crate::arena::numbig::NumBigArena> {
        self.graph_indexes
            .graphs
            .get(&g_id)
            .and_then(|gi| gi.numbig.get(&p_id))
    ...
    fn vector_arena_for_graph(
        ...
        g_id: GraphId,
        p_id: u32,
    ) -> Option<&crate::arena::vector::LazyVectorArena> {
        self.graph_indexes
            .graphs
            .get(&g_id)
...
impl BinaryGraphView {
    ...
    pub fn numbig_arena(&self, p_id: u32) -> Option<&crate::arena::numbig::NumBigArena> {
        self.store.numbig_arena_for_graph(self.g_id, p_id)
    }

    /// Get the vector arena for this graph and predicate.
    pub fn vector_arena(&self, p_id: u32) -> Option<&crate::arena::vector::LazyVectorArena> {
...
fn cache_bytes_to_path_atomic(target: &Path, bytes: &[u8]) -> io::Result<()> {
    ...
    if let Err(_rename_err) = std::fs::rename(&tmp, target) {
        ...
        // or on Unix with different filesystems. Content-addressed: if target
        // now exists, we can discard our tmp.
