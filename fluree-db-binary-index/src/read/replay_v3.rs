//! V3 time-travel replay: reconstruct leaflet state at `t_target` using
//! base columns (latest-state) and history sidecar entries.
//!
//! Equivalent to `replay.rs` (V5) but uses V3 identity model:
//! - **Fact identity**: `(s_id, p_id, o_type, o_key, o_i)`
//! - **History entries**: `HistEntryV2` from FHS1 sidecar (not inline Region 3)
//! - **Input/output**: `ColumnBatch` (not `RowColumnSlice` / `ReplayedLeaflet`)
//!
//! ## Algorithm (same 4-step structure as V5)
//!
...
mod tests {
    ...
    fn no_replay_needed() {
        ...
        let result = replay_leaflet_v3(&batch, &[], 5, RunSortOrder::Spot);
    ...
    fn exclude_row_above_target() {
        ...
        let result = replay_leaflet_v3(&batch, &[], 3, RunSortOrder::Spot);
    ...
    fn restore_retracted_fact() {
        ...
        let result = replay_leaflet_v3(&batch, &history, 4, RunSortOrder::Spot);
        assert!(result.is_some());
        let replayed = result.unwrap();
    ...
    fn swap_newer_base_row_with_older_version() {
        ...
        let result = replay_leaflet_v3(&batch, &history, 3, RunSortOrder::Spot);
        assert!(result.is_some());
        let replayed = result.unwrap();
        assert_eq!(replayed.row_count, 1);
        assert_eq!(replayed.s_id.get(0), 1);
