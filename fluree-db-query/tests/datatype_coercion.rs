...
#[test]
fn coerce_strings() {
    assert_eq!(coerce(serde_json::json!("foo"), xsd::STRING), Some(serde_json::json!("foo")));
    assert_eq!(coerce(serde_json::json!(42), xsd::STRING), None);
}

#[test]
fn coerce_id() {
    assert_eq!(coerce(serde_json::json!("foo"), "@id"), Some(serde_json::json!("foo")));
    assert_eq!(coerce(serde_json::json!(42), "@id"), Some(serde_json::json!(42)));
}

#[test]
fn coerce_boolean() {
    assert_eq!(coerce(serde_json::json!("true"), xsd::BOOLEAN), Some(serde_json::json!(true)));
    assert_eq!(coerce(serde_json::json!("false"), xsd::BOOLEAN), Some(serde_json::json!(false)));
    assert_eq!(coerce(serde_json::json!(true), xsd::BOOLEAN), Some(serde_json::json!(true)));
    assert_eq!(coerce(serde_json::json!(false), xsd::BOOLEAN), Some(serde_json::json!(false)));
...
#[test]
fn coerce_datetime() {
    assert_eq!(coerce(serde_json::json!("1980-10-5T11:23:00Z"), xsd::DATE_TIME), None);
    assert_eq!(
        coerce(serde_json::json!("1980-10-05T11:23:00-06:00"), xsd::DATE_TIME),
...
#[test]
fn coerce_decimal() {
    assert_eq!(coerce(serde_json::json!(3.14), xsd::DECIMAL), Some(serde_json::json!(3.14)));
    assert_eq!(coerce(serde_json::json!("3.14"), xsd::DECIMAL), Some(serde_json::json!("3.14")));
...
#[test]
fn coerce_double() {
    assert_eq!(coerce(serde_json::json!("INF"), xsd::DOUBLE), Some(serde_json::json!(f64::INFINITY)));
    assert_eq!(coerce(serde_json::json!("-INF"), xsd::DOUBLE), Some(serde_json::json!(f64::NEG_INFINITY)));
    assert_eq!(coerce(serde_json::json!("foo"), xsd::DOUBLE), None);
}

#[test]
fn coerce_float() {
    assert_eq!(coerce(serde_json::json!("INF"), xsd::FLOAT), Some(serde_json::json!(f64::INFINITY)));
    assert_eq!(coerce(serde_json::json!("-INF"), xsd::FLOAT), Some(serde_json::json!(f64::NEG_INFINITY)));
    assert_eq!(coerce(serde_json::json!("foo"), xsd::FLOAT), None);
}

#[test]
fn coerce_integer() {
    assert_eq!(coerce(serde_json::json!(42), xsd::INTEGER), Some(serde_json::json!(42)));
    assert_eq!(coerce(serde_json::json!("42"), xsd::INTEGER), Some(serde_json::json!(42)));
...
#[test]
fn coerce_int() {
    assert_eq!(coerce(serde_json::json!(42), xsd::INT), Some(serde_json::json!(42)));
...
fn coerce_unsigned_int() {
    assert_eq!(coerce(serde_json::json!(42), xsd::UNSIGNED_INT), Some(serde_json::json!(42)));
    assert_eq!(coerce(serde_json::json!(-42), xsd::UNSIGNED_INT), None);
}

#[test]
fn coerce_other_integer_types() {
    assert_eq!(coerce(serde_json::json!(42), xsd::LONG), Some(serde_json::json!(42)));
    assert_eq!(coerce(serde_json::json!(42), xsd::SHORT), Some(serde_json::json!(42)));
    assert_eq!(coerce(serde_json::json!(42), xsd::BYTE), Some(serde_json::json!(42)));
    assert_eq!(coerce(serde_json::json!(42), xsd::UNSIGNED_LONG), Some(serde_json::json!(42)));
    assert_eq!(coerce(serde_json::json!(42), xsd::UNSIGNED_SHORT), Some(serde_json::json!(42)));
    assert_eq!(coerce(serde_json::json!(42), xsd::UNSIGNED_BYTE), Some(serde_json::json!(42)));
    assert_eq!(coerce(serde_json::json!(42), xsd::POSITIVE_INTEGER), Some(serde_json::json!(42)));
    assert_eq!(coerce(serde_json::json!(-1), xsd::NEGATIVE_INTEGER), Some(serde_json::json!(-1)));
    assert_eq!(coerce(serde_json::json!(0), xsd::NON_POSITIVE_INTEGER), Some(serde_json::json!(0)));
    assert_eq!(coerce(serde_json::json!(0), xsd::NON_NEGATIVE_INTEGER), Some(serde_json::json!(0)));
    assert_eq!(coerce(serde_json::json!(-1), xsd::POSITIVE_INTEGER), None);
    assert_eq!(coerce(serde_json::json!(1), xsd::NON_POSITIVE_INTEGER), None);
}

#[test]
fn coerce_string_types() {
    assert_eq!(coerce(serde_json::json!("foo"), xsd::NORMALIZED_STRING), Some(serde_json::json!("foo")));
    assert_eq!(coerce(serde_json::json!("foo"), xsd::TOKEN), Some(serde_json::json!("foo")));
    assert_eq!(coerce(serde_json::json!("en"), xsd::LANGUAGE), Some(serde_json::json!("en")));
...
fn coerce_non_coerced_datatypes() {
    assert_eq!(coerce(serde_json::json!("0F0A"), xsd::HEX_BINARY), Some(serde_json::json!("0F0A")));
    assert_eq!(coerce(serde_json::json!("P1DT2H"), xsd::DURATION), Some(serde_json::json!("P1DT2H")));
}
