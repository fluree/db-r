...
use crate::fast_path_common::{
    ...
    leaf_entries_for_predicate, normalize_pred_sid, predicate_range_keys, projection_okey_only,
...
impl Operator for PredicateChainExistsJoinCountAllOperator {
    ...
    async fn open(&mut self, ctx: &ExecutionContext<'_>) -> Result<()> {
        ...
        if let Some(store) = fast_path_store(ctx) {
            if let Some(count) =
                count_chain_exists_join(store, ctx.binary_g_id, &self.p1, &self.p2, &self.p3)?
            {
...
/// `(object_key, row_count)` groups in sorted object order.
///
/// Requires `o_type_const == IRI_REF` on every leaflet — returns `None` from
/// `new()` if any leaflet has a different or missing `o_type_const`.
