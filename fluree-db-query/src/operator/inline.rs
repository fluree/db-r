...
//!
//! [`InlineOperator`] and [`apply_inline`] support inlining eligible filters
//! and binds into host operators so they execute per-row without separate
//! wrapper nodes.
...
///
/// Replaces separate `FilterOperator` and `BindOperator` wrapper nodes for
/// eligible operations whose required variables are all bound by the time the
/// host operator executes.
#[derive(Debug, Clone)]
