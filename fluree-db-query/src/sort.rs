...
fn materialize_encoded_lit_for_sort(
    b: &Binding,
...
pub fn compare_bindings(a: &Binding, b: &Binding) -> Ordering {
    match (a, b) {
        ...
        (Binding::EncodedLit { .. }, Binding::Unbound | Binding::Poisoned | Binding::Sid(_) | Binding::IriMatch { .. } | Binding::Iri(_)) => {
            Ordering::Greater
        }
        (Binding::EncodedLit { .. }, Binding::Lit { .. }) => Ordering::Equal,
        (Binding::Lit { .. }, Binding::EncodedLit { .. }) => Ordering::Equal,
        (Binding::EncodedLit { o_kind: k1, o_key: ok1, p_id: p1, dt_id: dt1, lang_id: l1, i_val: i1, .. },
         Binding::EncodedLit { o_kind: k2, o_key: ok2, p_id: p2, dt_id: dt2, lang_id: l2, i_val: i2, .. }) => {
            k1.cmp(k2)
                .then(ok1.cmp(ok2))
                .then(p1.cmp(p2))
                .then(dt1.cmp(dt2))
