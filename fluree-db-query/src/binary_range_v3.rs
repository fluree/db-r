...
fn binary_lookup_subject_predicate_refs_batched_v3(
    ...
    // Only translate flakes that match the target predicate and subject set.
    {
        ...
        if !ops.is_empty() {
            fluree_db_binary_index::read::types_v3::sort_overlay_ops_v3(
                &mut ops,
                RunSortOrder::Psot,
            );
            cursor.set_overlay_ops(ops);
...
fn batched_refs_overlay_only(
    ...
    overlay.for_each_overlay_flake(
        ...
        &mut |flake| {
            ...
            if let Ok(op) = crate::binary_scan_v3::translate_one_flake_v3_pub(
                flake,
                store,
                Some(dict_novelty),
                &mut ephemeral_preds,
...
fn binary_range_bounded_v3(
    ...
    let filter = BinaryFilterV3::default();
    let projection = ColumnProjection::all();
    let mut cursor = BinaryCursorV3::new(
        Arc::clone(store),
        order,
    ...
    if !overlay_ops.is_empty() {
        fluree_db_binary_index::read::types_v3::sort_overlay_ops_v3(&mut overlay_ops, order);
