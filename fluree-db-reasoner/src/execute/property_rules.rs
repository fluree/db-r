...
use super::util::{ref_dt, try_derive_type};
    ...
    for p in ontology.symmetric_properties() {
        for flake in delta.get_by_p(p) {
            ...
            if let FlakeValue::Ref(y) = &flake.o {
                ...
                if !derived.contains(&inverse.s, &inverse.p, &inverse.o) {
                    new_delta.push(inverse);
                    diagnostics.record_rule_fired("prp-symp");
                }
            }
        }
    }
    ...
    for p in ontology.transitive_properties() {
        for flake in delta.get_by_p(p) {
            if let FlakeValue::Ref(y) = &flake.o {
                ...
                for existing in derived.get_by_ps(p, y) {
                    if let FlakeValue::Ref(z) = &existing.o {
                        ...
                        if x != z {
                            ...
                            if !derived.contains(&new_flake.s, &new_flake.p, &new_flake.o) {
                                new_delta.push(new_flake);
                                diagnostics.record_rule_fired("prp-trp");
                            }
                        }
                    }
                }
                ...
                for delta_flake in delta.get_by_ps(p, y) {
                ...
                for existing in derived.get_by_po(p, x) {
                    let w = &existing.s;
    ...
    for flake in delta.iter() {
        ...
        if let FlakeValue::Ref(y) = &flake.o {
            ...
            for p_inv in inverses {
                ...
                if !derived.contains(&inverse.s, &inverse.p, &inverse.o) {
                    new_delta.push(inverse);
                    diagnostics.record_rule_fired("prp-inv");
                }
            }
        }
    }
    ...
    for p in ontology.properties_with_range() {
        ...
        for flake in delta.get_by_p(p) {
            ...
            if let FlakeValue::Ref(y) = &flake.o {
                ...
                let canonical_y = same_as.canonical(y);
    ...
    for p1 in ontology.properties_with_super_properties() {
        ...
        for flake in delta.get_by_p(p1) {
            ...
            let canonical_o = match &flake.o {
                FlakeValue::Ref(o) => FlakeValue::Ref(same_as.canonical(o)),
                other => other.clone(),
            };
            ...
            for p2 in super_props {
                ...
                if !derived.contains(&derived_flake.s, &derived_flake.p, &derived_flake.o) {
                    ...
                    diagnostics.record_rule_fired("prp-spo1");
                }
            }
        }
    }
...
pub fn apply_property_chain_rule(
    ontology: &OntologyRL,
    delta: &DeltaSet,
    derived: &DerivedSet,
    new_delta: &mut DeltaSet,
    ...
    for chain in ontology.property_chains() {
        apply_single_property_chain(chain, delta, derived, new_delta, same_as, t, diagnostics);
    }
}

/// Apply a single property chain rule.
fn apply_single_property_chain(
    chain: &PropertyChain,
    delta: &DeltaSet,
    derived: &DerivedSet,
    new_delta: &mut DeltaSet,
    ...
    for seed_pos in 0..chain_len {
        ...
        for seed_flake in delta.get_by_p(&element.property) {
            ...
            let (binding_in, binding_out) = if element.is_inverse {
                ...
                match &seed_flake.o {
                    FlakeValue::Ref(obj) => {
                        (same_as.canonical(obj), same_as.canonical(&seed_flake.s))
                    }
                ...
                match &seed_flake.o {
                    FlakeValue::Ref(obj) => {
                        (same_as.canonical(&seed_flake.s), same_as.canonical(obj))
                    }
                    _ => continue, // Non-ref objects can't be part of chain
                }
            };
    ...
    for pos in (0..seed_pos).rev() {
        ...
        for node in &current_nodes {
            ...
            let candidates = if element.is_inverse {
                ...
                get_all_objects_for_subject(delta, derived, &element.property, node, same_as)
            } else {
                // Need subjects for object = u_{pos+1}
                get_all_subjects_for_object(delta, derived, &element.property, node, same_as)
            };
...
fn get_all_subjects_for_object(
    ...
    for flake in delta.get_by_po(predicate, object) {
        subjects.push(same_as.canonical(&flake.s));
    }
...
fn get_all_objects_for_subject(
    ...
    for flake in delta.get_by_ps(predicate, subject) {
        if let FlakeValue::Ref(obj) = &flake.o {
            objects.push(same_as.canonical(obj));
        }
    }
...
fn derive_chain_fact(
    ...
    let canonical_start = same_as.canonical(start);
    let canonical_end = FlakeValue::Ref(same_as.canonical(end));

    // Check if already in derived
    if derived.contains(&canonical_start, derived_prop, &canonical_end) {
        return;
    }

    // Check if already in new_delta (we might seed from multiple positions)
    for existing in new_delta.get_by_ps(derived_prop, &canonical_start) {
    ...
    new_delta.push(chain_flake);
    diagnostics.record_rule_fired("prp-spo2");
}
...
/// # Arguments
///
/// * `same_as_changed` - If true, re-evaluate even without new P facts in delta.
...
pub fn apply_functional_property_rule(
    ontology: &OntologyRL,
    delta: &DeltaSet,
    derived: &DerivedSet,
    new_delta: &mut DeltaSet,
    ...
    for p in ontology.functional_properties() {
        ...
        for flake in derived.get_by_p(p) {
            if let FlakeValue::Ref(y) = &flake.o {
                let x_canonical = same_as.canonical(&flake.s);
                let y_canonical = same_as.canonical(y);
                objects_by_subject
        ...
        for (_x, objects) in objects_by_subject {
            ...
            for other in &objects_vec[1..] {
                ...
                if !derived.contains(&same_as_flake.s, &same_as_flake.p, &same_as_flake.o) {
                    ...
                    diagnostics.record_rule_fired("prp-fp");
                }
            }
        }
    }
    ...
    for p in ontology.inverse_functional_properties() {
        ...
        for (_y, subjects) in subjects_by_object {
            ...
            for other in &subjects_vec[1..] {
                ...
                if !derived.contains(&same_as_flake.s, &same_as_flake.p, &same_as_flake.o) {
                    new_delta.push(same_as_flake);
                    diagnostics.record_rule_fired("prp-ifp");
                }
            }
        }
    }
...
/// # Arguments
///
/// * `same_as_changed` - If true, re-evaluate even without new relevant facts in delta.
///   This is needed because sameAs merges can create new key matches after canonicalization.
pub fn apply_has_key_rule(
    ontology: &OntologyRL,
    delta: &DeltaSet,
    derived: &DerivedSet,
    new_delta: &mut DeltaSet,
    ...
    for class in ontology.classes_with_keys() {
        for key_properties in ontology.has_keys_of(class) {
            ...
            let delta_has_new_type = delta
                .get_by_p(rdf_type_sid)
                .any(|f| matches!(&f.o, FlakeValue::Ref(c) if same_as.canonical(c) == same_as.canonical(class)));
            ...
            let canonical_class = same_as.canonical(class);
            let mut instances: HashSet<Sid> = HashSet::new();

            // From delta: type(x, C)
            for flake in delta.get_by_p(rdf_type_sid) {
                if let FlakeValue::Ref(c) = &flake.o {
                    if same_as.canonical(c) == canonical_class {
                        instances.insert(same_as.canonical(&flake.s));
                    }
                }
            }

            // From derived: type(x, C)
            for flake in derived.get_by_p(rdf_type_sid) {
                if let FlakeValue::Ref(c) = &flake.o {
                    if same_as.canonical(c) == canonical_class {
                        instances.insert(same_as.canonical(&flake.s));
                    }
            ...
            for instance in &instances {
                ...
                for key_prop in key_properties {
                    ...
                    // From delta
                    for flake in delta.get_by_ps(key_prop, instance) {
                        if let FlakeValue::Ref(v) = &flake.o {
                            canonical_values.insert(same_as.canonical(v));
                        }
                        // Skip literals - restricted form only supports Ref values
                    }

                    // From derived
                    for flake in derived.get_by_ps(key_prop, instance) {
                        if let FlakeValue::Ref(v) = &flake.o {
                            canonical_values.insert(same_as.canonical(v));
                        }
                    }
            ...
            for (_key_tuple, matching_instances) in instances_by_key {
                ...
                for other in &matching_instances[1..] {
                    ...
                    if !derived.contains(&same_as_flake.s, &same_as_flake.p, &same_as_flake.o) {
                        new_delta.push(same_as_flake);
                        diagnostics.record_rule_fired("prp-key");
                    }
                }
            }
        }
