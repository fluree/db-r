...
async fn commit_db_metadata_spo_queries_parity() {
    ...
    for row in rows {
        ...
        let subject = arr[0].as_str().unwrap_or_default();
        assert!(
            subject.starts_with("fluree:db:"),
            "unexpected db subject: {}",
            subject
        );
        let t_val = &arr[1];
    ...
    for row in rows {
        ...
        assert!(
            commit.starts_with("fluree:commit:"),
            "unexpected commit ref: {}",
            commit
        );
