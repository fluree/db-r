...
use crate::{NodeCache, OverlayProvider, Storage};
...
/// ```
#[derive(Clone)]
pub struct FlureeDataSetView<S: Storage + 'static, C: NodeCache + 'static> {
    /// Default graph views (merged for queries without GRAPH clause)
    pub default: Vec<FlureeView<S, C>>,
    /// Named graph views (accessed via GRAPH clause)
    pub named: HashMap<Arc<str>, FlureeView<S, C>>,
...
impl<S: Storage + Clone + 'static, C: NodeCache + 'static> FlureeDataSetView<S, C> {
    ...
    pub fn single(view: FlureeView<S, C>) -> Self {
        Self {
            default: vec![view],
            named: HashMap::new(),
            named_order: Vec::new(),
    ...
    pub fn with_default(mut self, view: FlureeView<S, C>) -> Self {
        self.default.push(view);
        self
    }

    /// Add a view as a named graph.
    pub fn with_named(mut self, name: impl Into<Arc<str>>, view: FlureeView<S, C>) -> Self {
    ...
    pub fn with_defaults(mut self, views: impl IntoIterator<Item = FlureeView<S, C>>) -> Self {
    ...
    /// - else None
    pub fn primary(&self) -> Option<&FlureeView<S, C>> {
    ...
    pub fn primary_mut(&mut self) -> Option<&mut FlureeView<S, C>> {
    ...
    pub fn get_named(&self, name: &str) -> Option<&FlureeView<S, C>> {
        self.named.get(name)
    }
    ...
    pub(crate) fn as_runtime_dataset<'a>(&'a self) -> fluree_db_query::DataSet<'a, S, C> {
    ...
    pub fn into_single(mut self) -> Option<FlureeView<S, C>> {
...
impl<S: Storage + Clone + 'static, C: NodeCache + 'static> Default for FlureeDataSetView<S, C> {
    fn default() -> Self {
        Self::new()
    }
}

impl<S: Storage + Clone + 'static, C: NodeCache + 'static> std::fmt::Debug
    for FlureeDataSetView<S, C>
{
