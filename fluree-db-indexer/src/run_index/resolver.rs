...
impl CommitResolver {
    ...
    pub fn resolve_commit_ops<W: RecordSink>(
        ...
        let span = tracing::trace_span!("resolve_commit", t = t, ops = tracing::field::Empty,);
    ...
    fn resolve_predicate(&mut self, ns_code: i32, name: &str, dicts: &mut GlobalDicts) -> u32 {
        let prefix = self.lookup_prefix(ns_code);
...
mod tests {
    ...
    fn test_emit_txn_meta() {
        ...
        assert_eq!(
            time_rec.dt,
            dt_ids::LONG,
            "ledger:time must be dt_ids::LONG"
        );
        assert_eq!(
            time_rec.o_kind,
        ...
        assert_eq!(
            t_rec.dt,
            dt_ids::INTEGER,
            "ledger:t must be dt_ids::INTEGER"
        );
        assert_eq!(
            t_rec.o_kind,
            ObjKind::NUM_INT.as_u8(),
        ...
        assert_eq!(
            prev_rec.dt,
            dt_ids::ID,
            "ledger:previous must be dt_ids::ID"
        );
        assert_eq!(
            prev_rec.o_kind,
