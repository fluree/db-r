...
impl BinaryIndexStore {
    ...
    /// Translate a FlakeValue to a ValueId for index key matching.
    ///
    /// Returns `None` if the value cannot be translated (e.g., string without
    /// reverse index, unsupported float encoding).
    pub fn value_to_value_id(&self, val: &FlakeValue) -> io::Result<Option<ValueId>> {
        match val {
            FlakeValue::Null => Ok(Some(ValueId::NULL)),
            FlakeValue::Boolean(b) => Ok(Some(if *b { ValueId::BOOL_TRUE } else { ValueId::BOOL_FALSE })),
            FlakeValue::Long(n) => Ok(ValueId::num_int(*n)),
            FlakeValue::Ref(sid) => {
                // IRI ref: look up s_id in subject reverse index
                match self.sid_to_s_id(sid)? {
                    Some(s_id) => Ok(Some(ValueId::iri_id(s_id))),
                    None => Ok(None), // IRI not in this index
                }
            }
            FlakeValue::String(s) => {
                // String literal: look up str_id in string reverse index
                match self.find_string_id(s)? {
                    Some(str_id) => Ok(Some(ValueId::lex_id(str_id))),
                    None => Ok(None), // string not in this index
                }
            }
            // Float/Date/Time/DateTime: not yet supported for key translation.
            _ => Ok(None),
        }
    }
    ...
    pub fn translate_range(
        ...
        let s_id = match s {
            Some(sid) => match self.sid_to_s_id(sid)? {
                Some(id) => Some(id),
                None => return Ok(None), // subject not in index
            },
            None => None,
        };
        ...
        let p_id = match p {
            Some(sid) => match self.sid_to_p_id(sid) {
                Some(id) => Some(id),
                None => return Ok(None), // predicate not in index
            },
            None => None,
        };

        let o_vid = match o {
            Some(val) => match self.value_to_value_id(val)? {
                Some(vid) => Some(vid),
                None => return Ok(None), // value not translatable
            },
            None => None,
        };

        // Build min/max RunRecord bounds
    ...
    pub fn row_to_flake(
        ...
        // Object: ValueId → FlakeValue
        let vid = ValueId::from_u64(o_raw);
        let o_val = self.decode_value(vid)?;
    ...
    /// Decode a ValueId into a FlakeValue.
    pub fn decode_value(&self, vid: ValueId) -> io::Result<FlakeValue> {
        let tag = vid.tag();
        let payload = vid.payload();

        match tag {
            0x0 => Ok(FlakeValue::Null), // MIN sentinel
            0x1 => Ok(FlakeValue::Null), // NULL
            0x2 => Ok(FlakeValue::Boolean(payload != 0)), // BOOL
            0x3 => Ok(FlakeValue::Long(vid.decode_offset_binary())), // NUM_INT
            0x4 => {
                // NUM_FLOAT — not yet supported in Phase C query path.
                Err(io::Error::new(
                    io::ErrorKind::Unsupported,
                    format!("NUM_FLOAT decoding not implemented (payload={})", payload),
                ))
            }
            0x5 => {
                // IRI_ID → resolve subject IRI → Ref(Sid)
                let ref_s_id = payload as u32;
                let ref_iri = self.resolve_subject_iri(ref_s_id)?;
                Ok(FlakeValue::Ref(self.encode_iri(&ref_iri)))
            }
            0x6 => {
                // LEX_ID → resolve string value
                let str_id = payload as u32;
                let s = self.resolve_string_value(str_id)?;
                Ok(FlakeValue::String(s))
            }
            0x7 => {
                // DATE → days since epoch → ISO string → parse
                let days = vid.decode_offset_binary() as i32;
            ...
            0x8 => {
                // TIME → micros since midnight → ISO string → parse
                let micros = payload as i64;
            ...
            0x9 => {
                // DATETIME → epoch micros → ISO string → parse
                let epoch_micros = vid.decode_offset_binary();
            ...
            0xB => {
                // JSON → resolve string value
                let str_id = payload as u32;
                let s = self.resolve_string_value(str_id)?;
                Ok(FlakeValue::Json(s))
            }
            0xF => Ok(FlakeValue::Null), // MAX sentinel
            _ => Ok(FlakeValue::Null),
        }
    }
