...
//!   [LeafletDirectory: leaflet_count × 24 bytes]
//!     offset: u64, compressed_len: u32, row_count: u32, first_s_id: u32, first_p_id: u32
...
/// Per-leaflet directory entry: offset(8) + compressed_len(4) + row_count(4) + first_s_id(4) + first_p_id(4) = 24.
const LEAFLET_DIR_ENTRY: usize = 24;
...
/// Sort key written size in bytes (padded to 28 for alignment in leaf headers).
...
impl LeafWriter {
    ...
    fn flush_leaflet(&mut self) -> io::Result<()> {
        ...
        let first_s_id = self.record_buf[0].s_id;
...
pub fn read_leaf_header(data: &[u8]) -> io::Result<LeafFileHeader> {
    ...
    for _ in 0..leaflet_count {
        leaflet_dir.push(LeafletDirEntry {
            ...
            first_s_id: u32::from_le_bytes(data[pos + 16..pos + 20].try_into().unwrap()),
            first_p_id: u32::from_le_bytes(data[pos + 20..pos + 24].try_into().unwrap()),
        });
...
mod tests {
    ...
    fn test_leaf_writer_single_leaf() {
        ...
        assert_eq!(infos[0].first_key.s_id, 0);
        assert_eq!(infos[0].last_key.s_id, 14);
    ...
    fn test_leaf_file_round_trip() {
        ...
        assert_eq!(all_s_ids, (0..9u32).collect::<Vec<_>>());
