...
//! 3. Untouched leaflets: passthrough (raw `EncodedLeafletV3` + payload bytes).
//! 4. Touched leaflets: decode columns → `merge_novelty_v3` → re-encode via
//!    `encode_leaflet_v3` (one encode per segmentation-safe chunk).
...
use fluree_db_binary_index::format::leaf_v3::{
    ...
    decode_leaf_header_v3, DecodedLeafDirV3, LeafInfoV3, LeafletDirEntryV3,
};
use fluree_db_binary_index::format::leaflet_v3::encode_leaflet_v3;
...
use super::novelty_merge_v3::{merge_novelty_v3, MergeInputV3, MergeOutputV3};
...
pub struct LeafUpdateInputV3<'a> {
    /// Raw FLI3 leaf bytes.
    pub leaf_bytes: &'a [u8],
...
enum EncodedLeafletInfo {
    ...
    Encoded(fluree_db_binary_index::format::leaflet_v3::EncodedLeafletV3),
}
...
/// `leaf_target_rows`).
pub fn update_leaf_v3(input: &LeafUpdateInputV3<'_>) -> io::Result<LeafUpdateOutputV3> {
...
fn passthrough_entire_leaf(input: &LeafUpdateInputV3<'_>) -> LeafUpdateOutputV3 {
...
fn merge_and_encode_leaflet(
    ...
    if chunks.len() == 1 {
        ...
        let encoded = encode_leaflet_v3(chunks[0], order, zstd_level);
        ...
        for (chunk, chunk_history) in chunks.iter().zip(partitioned) {
            let encoded = encode_leaflet_v3(chunk, order, zstd_level);
            result.push(ProcessedLeafletV3 {
                encoded: EncodedLeafletInfo::Encoded(encoded),
...
fn assemble_output_leaves(
    ...
    input: &LeafUpdateInputV3<'_>,
) -> io::Result<LeafUpdateOutputV3> {
    use fluree_db_binary_index::format::leaflet_v3::EncodedLeafletV3;

    if processed.is_empty() {
        return Ok(LeafUpdateOutputV3 { leaves: vec![] });
    }

    // Convert ProcessedLeafletV3 into (EncodedLeafletV3, Vec<HistEntryV2>) pairs
    // for assembly.
    let mut leaflet_data: Vec<(EncodedLeafletV3, Vec<HistEntryV2>)> =
    ...
    let mut current_group: Vec<(EncodedLeafletV3, Vec<HistEntryV2>)> = Vec::new();
    ...
    for group in leaves {
        ...
        output.push(NewLeafBlobV3 { info: leaf_info });
    }

    Ok(LeafUpdateOutputV3 { leaves: output })
}

/// Build a single leaf blob from a group of (EncodedLeafletV3, history) pairs.
fn build_leaf_from_group(
    group: Vec<(
        fluree_db_binary_index::format::leaflet_v3::EncodedLeafletV3,
        Vec<HistEntryV2>,
    )>,
    order: RunSortOrder,
) -> io::Result<LeafInfoV3> {
    use fluree_db_binary_index::format::leaflet_v3::EncodedLeafletV3;
    ...
    let owned_leaflets: Vec<EncodedLeafletV3> = group.into_iter().map(|(e, _)| e).collect();
    ...
    // For LeafInfoV3, first_key/last_key are RunRecordV2 — but for incremental
...
/// Reconstruct an `EncodedLeafletV3` from a passthrough directory entry + payload.
...
mod tests {
    ...
    fn test_update_no_novelty() {
        ...
        let output = update_leaf_v3(&input).unwrap();
