...
//! - `RunRecordV2` / `StreamingRunReaderV2` / `KWayMergeV2`
//! - `LeafWriterV3` (segmentation-aware, columnar)
//! - `build_branch_v3_bytes` (FBR3 with sidecar_cid)
//!
...
use super::merge_v2::KWayMergeV2;
use crate::run_index::runs::streaming_reader_v2::StreamingRunReaderV2;
use fluree_db_binary_index::format::branch_v3::{build_branch_v3_bytes, LeafEntryV3};
use fluree_db_binary_index::format::leaf_v3::{LeafInfoV3, LeafWriterV3};
...
#[derive(Debug)]
pub struct IndexBuildV2Result {
    pub graphs: Vec<GraphIndexResultV2>,
    pub total_rows: u64,
    pub index_dir: PathBuf,
...
pub fn build_index_v2(config: &IndexBuildV2Config) -> Result<IndexBuildV2Result, IndexBuildError> {
    let t0 = Instant::now();
    ...
    let streams: Vec<StreamingRunReaderV2> = run_paths
        .iter()
        .map(|p| StreamingRunReaderV2::open(p))
        .collect::<io::Result<Vec<_>>>()?;

    let cmp = cmp_v2_for_order(config.sort_order);
    let mut merge = KWayMergeV2::new(streams, cmp)?;
    ...
    let mut writer = LeafWriterV3::new(
        order,
...
fn finish_graph_v2(
    ...
    let leaf_entries: Vec<LeafEntryV3> = leaf_infos
        .iter()
        .map(|info| LeafEntryV3 {
...
fn discover_run_files_v2(dir: &Path) -> io::Result<Vec<PathBuf>> {
...
/// Configuration for `build_all_indexes_v2`.
#[derive(Debug, Clone)]
pub struct BuildAllV2Config {
...
pub fn build_all_indexes_v2(
    config: &BuildAllV2Config,
) -> Result<Vec<(RunSortOrder, IndexBuildV2Result)>, IndexBuildError> {
    ...
    for &order in orders {
        ...
        let order_config = IndexBuildV2Config {
            run_dir,
            index_dir: config.index_dir.clone(),
            sort_order: order,
        ...
        let result = build_index_v2(&order_config)?;
...
mod tests {
    ...
    fn build_single_order_post() {
        ...
        let result = build_index_v2(&config).unwrap();
