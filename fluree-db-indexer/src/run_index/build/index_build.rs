...
use crate::run_index::resolve::lang_remap::build_lang_remap;
use crate::run_index::runs::streaming_reader::StreamingRunReader;
use crate::run_index::vocab::dict_io::{read_predicate_dict, write_language_dict};
use fluree_db_binary_index::format::branch::{write_branch_v2, LeafEntry};
...
fn build_index_from_run_paths_inner(
    ...
    let mut history_scratch: Vec<fluree_db_binary_index::format::run_record::RunRecord> =
        Vec::new();
...
impl ClassBitsetTable {
    ...
    pub fn build_from_type_maps<S: crate::run_index::runs::spool::SubjectRemap>(
        inputs: &[(&Path, &S)],
    ) -> io::Result<Self> {
...
pub struct SortedCommitInput<
    S: crate::run_index::runs::spool::SubjectRemap,
    R: crate::run_index::runs::spool::StringRemap,
> {
...
>(
    ...
    use crate::run_index::runs::sorted_commit_reader::StreamingSortedCommitReader;
    use fluree_db_binary_index::format::run_record::cmp_g_spot;
...
mod tests {
    ...
    use fluree_db_binary_index::format::run_record::{cmp_spot, RunRecord};
    ...
    fn test_spot_from_sorted_commits() {
        ...
        use crate::run_index::runs::spool::sort_remap_and_write_sorted_commit;
