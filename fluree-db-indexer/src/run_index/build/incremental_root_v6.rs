...
//!
//! Provides a selective mutation builder that clones an existing `IndexRootV6`
...
use fluree_db_binary_index::format::index_root_v6::{
    DefaultGraphOrderV3, IndexRootV6, NamedGraphRoutingV3,
};
...
pub struct IncrementalRootBuilderV6 {
    root: IndexRootV6,
    replaced_cids: Vec<ContentId>,
}

impl IncrementalRootBuilderV6 {
    /// Create a builder from an existing root (cloned).
    pub fn from_old_root(root: IndexRootV6) -> Self {
        Self {
            root,
    ...
    /// replaced CIDs from `BranchUpdateResultV3` via `add_replaced_cids()`,
    /// which knows exactly which leaves were rewritten vs carried forward.
    pub fn set_default_graph_order(&mut self, order: RunSortOrder, leaves: Vec<LeafEntryV3>) {
            ...
            .find(|o| o.order == order)
        {
            ...
            self.root
                .default_graph_orders
                .push(DefaultGraphOrderV3 { order, leaves });
        }
    }
    ...
    pub fn set_named_graph_branch(
        ...
        if let Some(ng) = self.root.named_graphs.iter_mut().find(|ng| ng.g_id == g_id) {
            ...
            self.root.named_graphs.push(NamedGraphRoutingV3 {
                g_id,
    ...
    pub fn set_datatype_iris(&mut self, iris: Vec<String>) {
        ...
        self.root.o_type_table =
            IndexRootV6::build_o_type_table(&custom_dt_iris, &self.root.language_tags);
    ...
    pub fn set_language_tags(&mut self, tags: Vec<String>) {
        let custom_dt_iris: Vec<String> = self
            ...
            .cloned()
            .collect();
        self.root.o_type_table = IndexRootV6::build_o_type_table(&custom_dt_iris, &tags);
        self.root.language_tags = tags;
    }

    /// Update per-graph arena references.
    pub fn set_graph_arenas(&mut self, arenas: Vec<GraphArenaRefsV5>) {
    ...
    pub fn build(self) -> (IndexRootV6, Vec<ContentId>) {
