...
use super::run_file_v2::{write_run_file_v2, write_run_file_v2_with_op, RunFileInfoV2};
...
// Note: #[allow] instead of #[expect] — see run_file_v2.rs for rationale.
#[allow(dead_code)]
pub(crate) struct RunWriterV2WithOp {
    config: RunWriterV2Config,
    buffer: Vec<RunRecordV2>,
    op_buffer: Vec<u8>,
    run_count: u32,
...
#[allow(dead_code)]
impl RunWriterV2WithOp {
    pub(crate) fn new(config: RunWriterV2Config) -> Self {
        let cap = config.max_records().min(1_000_000);
        Self {
            buffer: Vec::with_capacity(cap),
    ...
    fn flush_buffer(&mut self) -> io::Result<()> {
        ...
        let handle = std::thread::spawn(move || {
            ...
            let info = write_run_file_v2_with_op(
                &run_path,
                &sorted_recs,
                &sorted_ops,
                order,
    ...
    /// Finish writing, flush remaining records, return all run file metadata.
    pub(crate) fn finish(mut self) -> io::Result<RunWriterV2Result> {
        if !self.buffer.is_empty() {
            self.flush_buffer()?;
        }
        self.join_pending_flush()?;
...
// ============================================================================
// MultiOrderRunWriterV2WithOp
// ============================================================================

/// Fans out `(RunRecordV2, op)` pairs to per-order `RunWriterV2WithOp` instances.
// Kept for: incremental rebuild path (not yet wired).
// Use when: rebuild pipeline fans out records with ops to multiple orders.
#[allow(dead_code)]
pub(crate) struct MultiOrderRunWriterV2WithOp {
    writers: Vec<(RunSortOrder, RunWriterV2WithOp)>,
}

#[allow(dead_code)]
impl MultiOrderRunWriterV2WithOp {
    pub(crate) fn new(config: MultiOrderV2Config) -> io::Result<Self> {
        let per_order_budget = config.total_budget_bytes / config.orders.len().max(1);
        let mut writers = Vec::with_capacity(config.orders.len());
        ...
        for &order in &config.orders {
            let run_dir = config.base_run_dir.join(order.dir_name());
            std::fs::create_dir_all(&run_dir)?;
            let w = RunWriterV2WithOp::new(RunWriterV2Config {
                buffer_budget_bytes: per_order_budget,
                sort_order: order,
                run_dir,
            });
    ...
    /// Finish all writers and return per-order results.
    pub(crate) fn finish(self) -> io::Result<Vec<(RunSortOrder, RunWriterV2Result)>> {
        let mut results = Vec::with_capacity(self.writers.len());
        for (order, writer) in self.writers {
            results.push((order, writer.finish()?));
        }
...
mod tests {
    ...
    fn single_writer_with_op_basic() {
        let dir = std::env::temp_dir().join("fluree_test_run_writer_v2_with_op_basic");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();

        let mut writer = RunWriterV2WithOp::new(RunWriterV2Config {
            buffer_budget_bytes: 1024, // tiny budget to force multiple flushes
            sort_order: RunSortOrder::Spot,
            run_dir: dir.clone(),
        });
