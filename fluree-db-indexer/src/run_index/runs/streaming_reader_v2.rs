...
use super::run_file_v2::{RunFileHeaderV2, RUN_V2_HEADER_LEN, RUN_V2_VERSION_WITH_OP};
...
pub trait MergeSourceV2 {
    ...
    /// no op information. The `StreamingRunReaderV2WithOp` override returns
...
#[allow(dead_code)]
pub(crate) struct StreamingRunReaderV2WithOp {
    file: BufReader<std::fs::File>,
    /// Kept for the sort_order accessor.
    pub header: RunFileHeaderV2,
    buffer: Vec<RunRecordV2>,
...
#[allow(dead_code)]
impl StreamingRunReaderV2WithOp {
    ...
    pub(crate) fn open(path: &Path) -> io::Result<Self> {
        ...
        if header.version != RUN_V2_VERSION_WITH_OP {
            return Err(io::Error::new(
                ...
                format!(
                    "StreamingRunReaderV2WithOp requires version {}, got {}",
...
impl MergeSourceV2 for StreamingRunReaderV2WithOp {
    #[inline]
    fn peek(&self) -> Option<&RunRecordV2> {
        self.buffer.get(self.buf_pos)
    }
...
mod tests {
    use super::*;
    use crate::run_index::runs::run_file_v2::{write_run_file_v2, write_run_file_v2_with_op};
    ...
    fn streaming_read_with_op_roundtrip() {
        ...
        // Stream read with op.
        let mut reader = StreamingRunReaderV2WithOp::open(&path).unwrap();
        let mut read_back = Vec::new();
    ...
    fn default_peek_op_is_one() {
        // StreamingRunReaderV2 (no-op) should return peek_op() == 1 by default.
        ...
        write_run_file_v2(&path, &records, RunSortOrder::Spot, 1, 1).unwrap();

        let reader = StreamingRunReaderV2::open(&path).unwrap();
