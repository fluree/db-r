...
//!
//! A `Sid64` encodes both a namespace code and a per-namespace local ID into
//! a single `u64`:
//!
//! ```text
...
//!
//! The index root declares a `SidEncoding` that determines how `Sid64` values
...
#[repr(transparent)]
pub struct Sid64(pub u64);

impl Sid64 {
    /// Create a new `Sid64` from namespace code and local ID.
    ///
    /// # Panics
    ///
    ...
    /// Promote a narrow `u32` (`ns16_local16`) to a full `Sid64`.
    #[inline]
    ...
    /// Minimum possible `Sid64` (namespace 0, local 0).
    #[inline]
    pub fn min() -> Self {
        Self(0)
    }

    /// Maximum possible `Sid64`.
    #[inline]
    pub fn max() -> Self {
        Self(u64::MAX)
    }
...
impl fmt::Debug for Sid64 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Sid64(ns={}, local={})", self.ns_code(), self.local_id())
    }
}

impl fmt::Display for Sid64 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}:{}", self.ns_code(), self.local_id())
    }
}

impl From<u64> for Sid64 {
    #[inline]
    fn from(v: u64) -> Self {
        Self(v)
    }
...
impl From<Sid64> for u64 {
    #[inline]
    fn from(s: Sid64) -> Self {
        s.0
    }
}
...
impl Serialize for Sid64 {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        self.0.serialize(serializer)
    }
}

impl<'de> Deserialize<'de> for Sid64 {
...
///
/// Declared in the index root; determines how `Sid64` values are
/// read from and written to persistent storage.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum SidEncoding {
...
impl Default for SidEncoding {
    fn default() -> Self {
        Self::Narrow
    }
}

// === SidColumn: compact array of subject IDs ===

/// Compact array of subject IDs, stored as either narrow (`u32`) or wide (`Sid64`).
///
...
///
/// Accessing an element always returns [`Sid64`] — narrow values are promoted
/// via [`Sid64::from_sid32()`].
#[derive(Clone, Debug)]
pub enum SidColumn {
    /// Narrow: 4 bytes per element. `u32 = (ns_code << 16) | local_id`.
    Narrow(Arc<[u32]>),
    /// Wide: 8 bytes per element. Full Sid64 values.
    Wide(Arc<[Sid64]>),
}
...
impl SidColumn {
    ...
    /// Get the Sid64 at the given index.
    ///
    /// Narrow values are promoted via `Sid64::from_sid32()`.
    #[inline]
    pub fn get(&self, i: usize) -> Sid64 {
        match self {
            Self::Narrow(v) => Sid64::from_sid32(v[i]),
            Self::Wide(v) => v[i],
        }
    }
    ...
    /// Construct from decoded `u64` values (raw Sid64 bit patterns).
    ///
    ...
    pub fn from_u64_vec(values: Vec<u64>, encoding: SidEncoding) -> Self {
        match encoding {
            SidEncoding::Narrow => {
                let narrow: Vec<u32> = values
                    .iter()
                    .map(|&v| {
                        let sid = Sid64::from_u64(v);
                        sid.to_sid32()
            ...
            SidEncoding::Wide => {
                let wide: Vec<Sid64> = values.into_iter().map(Sid64::from_u64).collect();
                Self::Wide(wide.into())
            }
        }
    }
    ...
    /// Construct a wide column directly from Sid64 values.
    pub fn from_wide(values: Vec<Sid64>) -> Self {
...
mod tests {
    ...
    fn test_max_local_id() {
        ...
        let sid = Sid64::new(0xFFFF, max_local);
    ...
    fn test_ordering_local_within_namespace() {
        let a = Sid64::new(5, 10);
        let b = Sid64::new(5, 20);
        assert!(a < b);
    }

    #[test]
    fn test_sid32_round_trip() {
        let sid = Sid64::new(7, 1000);
        let sid32 = sid.to_sid32().expect("should fit in narrow");
        let restored = Sid64::from_sid32(sid32);
        assert_eq!(restored, sid);
    }

    #[test]
    fn test_sid32_overflow() {
        let sid = Sid64::new(1, 0x1_0000); // local_id = 65536, exceeds u16
        assert!(sid.to_sid32().is_none());
    }

    #[test]
    fn test_sid32_max_narrow() {
        let sid = Sid64::new(0xFFFF, 0xFFFF);
        let sid32 = sid.to_sid32().unwrap();
        assert_eq!(sid32, u32::MAX);
        assert_eq!(Sid64::from_sid32(sid32), sid);
    }

    #[test]
    fn test_min_max() {
        assert_eq!(Sid64::min().as_u64(), 0);
        assert_eq!(Sid64::max().as_u64(), u64::MAX);
        assert!(Sid64::min() < Sid64::new(0, 1));
        assert!(Sid64::new(u16::MAX, 0x0000_FFFF_FFFF_FFFF) <= Sid64::max());
    }
    ...
    fn test_serde_round_trip() {
        ...
        let parsed: Sid64 = serde_json::from_str(&json).unwrap();
        assert_eq!(parsed, sid);
    }

    #[test]
    fn test_sid_column_narrow() {
        let col = SidColumn::from_u64_vec(
            vec![
                Sid64::new(2, 10).as_u64(),
                Sid64::new(2, 20).as_u64(),
                Sid64::new(3, 5).as_u64(),
            ],
            SidEncoding::Narrow,
        );
        assert_eq!(col.len(), 3);
        assert_eq!(col.byte_size(), 12); // 3 × 4
        assert_eq!(col.get(0), Sid64::new(2, 10));
        assert_eq!(col.get(1), Sid64::new(2, 20));
        assert_eq!(col.get(2), Sid64::new(3, 5));
        assert!(matches!(col, SidColumn::Narrow(_)));
    }
    ...
    fn test_sid_column_wide() {
        let col = SidColumn::from_u64_vec(
            vec![
                Sid64::new(2, 0x1_0000).as_u64(), // exceeds u16
                Sid64::new(3, 5).as_u64(),
            ],
            SidEncoding::Wide,
        );
        ...
        assert_eq!(col.get(0), Sid64::new(2, 0x1_0000));
        assert_eq!(col.get(1), Sid64::new(3, 5));
        assert!(matches!(col, SidColumn::Wide(_)));
    }

    #[test]
    fn test_sid_encoding_serde() {
        let narrow = SidEncoding::Narrow;
        let json = serde_json::to_string(&narrow).unwrap();
        assert_eq!(json, "\"narrow\"");
        let parsed: SidEncoding = serde_json::from_str(&json).unwrap();
        assert_eq!(parsed, SidEncoding::Narrow);

        let wide = SidEncoding::Wide;
