    ...
    for i in 0..batch.num_rows {
        let matches = match (col, value) {
            (Column::Boolean(vals), LiteralValue::Boolean(lit)) => {
                vals.get(i).and_then(|v| *v).is_some_and(|v| compare_op(v, *lit, op))
            }
            ...
            (Column::Float32(vals), LiteralValue::Float32(lit)) => {
                vals.get(i).and_then(|v| *v).is_some_and(|v| compare_op_f32(v, *lit, op))
            }
            (Column::Float64(vals), LiteralValue::Float64(lit)) => {
                vals.get(i).and_then(|v| *v).is_some_and(|v| compare_op_f64(v, *lit, op))
            }
