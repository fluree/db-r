...
// Direct search (use the same language as the index)
let analyzer = Analyzer::english_default(); // or Analyzer::for_language(Language::French)
...
```json
{
    ...
    "query": { "kind": "bm25", "text": "rust programming", "language": "en" },
    "limit": 20,
    "as_of_t": 150,
    "sync": false,
    "timeout_ms": 5000
...
The BM25 query variant accepts an optional `language` field (BCP-47 tag) that controls which text analysis pipeline is used for query term processing. This should match the language used when the index was created.

```json
{ "kind": "bm25", "text": "les chats mangent", "language": "fr" }
```

If omitted, defaults to `"en"` (English). See [Supported Languages](../indexing-and-search/bm25.md#supported-languages) for the full list.

### Capabilities Response

The `/v1/capabilities` endpoint reports the service's supported analysis languages:

```json
{
    "protocol_version": "1.0",
    "bm25_analyzer_version": "english_default_v1",
    "supported_query_kinds": ["bm25"],
    "supported_languages": ["ar", "da", "de", "el", "en", "es", "fi", "fr", "hu", "it", "nb", "nl", "pt", "ro", "ru", "sv", "ta", "tr"],
    "max_limit": 1000,
...
- Analyzer configuration (tokenization, stemming, stopwords) per language
...
- **Config**: Stores the indexing query, BM25 parameters (k1, b), and language setting
