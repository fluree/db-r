...
BM25 indexes in Fluree are exposed as graph sources, allowing full-text search to be seamlessly integrated with graph queries using the same query interface.

## Overview

A BM25 graph source provides:
- Full-text search with relevance ranking
- Integration with structured graph queries
- Real-time index updates
- Standard query interface (SPARQL / JSON-LD)

## Creating BM25 Graph Sources
...
This creates graph source: `products-search:main`

## Graph Source Properties

BM25 graph sources expose special predicates:
...
BM25 graph sources update automatically:
