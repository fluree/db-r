...
## Legacy error code reference (not emitted by `fluree-server`)

This section is retained for historical reference. The current `fluree-server`
implementation does **not** emit `code` / `message` / `details` fields. Use the
`@type` field (compact error type IRI) and the human-readable `error` message.
