...
```json
{
  ...
  "index_t": 150,
  "hits": [
    { "iri": "ex:product-123", "ledger_address": "mydb:main", "score": 8.75 }
  ],
  "took_ms": 12
}
```

**Time travel note**: BM25 time-travel selection is implemented by BM25 itself (via a BM25-owned manifest/root in storage). The nameservice stores only a **head pointer** to the latest BM25 manifest/root (an opaque address) and does not store BM25 snapshot history.
