...
- Multi-language stemming and stopword filtering (18 languages, default English)
...
| `language` | `"en"` | BCP-47 language tag for text analysis. Determines which stemmer and stopword list are used. See [Supported Languages](#supported-languages). |
...
let config = Bm25CreateConfig::new("search", "docs:main", query)
    ...
    .with_k1(1.5)
    .with_b(0.5)
    .with_language("fr"); // French stemmer + stopwords
```

### Text Analysis

Fluree applies a language-aware text analysis pipeline:

1. **Tokenization**: Unicode-aware word boundary splitting (`\w+`)
2. **Lowercasing**: All tokens converted to lowercase
3. **Stopword filtering**: Language-specific common words removed
4. **Stemming**: Snowball stemmer for the configured language reduces words to root forms (e.g., English: "programming" -> "program", French: "mangent" -> "mang")

The analyzer language is set via the `language` configuration option (default: `"en"` for English). Both index-time and query-time analysis use the same pipeline to ensure consistent scoring.

### Supported Languages

BM25 indexes support all 18 Snowball stemming languages. Each language includes a dedicated stemmer and stopword list.

| BCP-47 | Language   | Example Stems |
|--------|------------|---------------|
| `ar`   | Arabic     | |
| `da`   | Danish     | "programmering" → "programmer" |
| `de`   | German     | "Katzen" → "katz", "Programmierung" → "programmier" |
| `el`   | Greek      | |
| `en`   | English    | "programming" → "program", "databases" → "databas" |
| `es`   | Spanish    | "gatos" → "gat", "programación" → "program" |
| `fi`   | Finnish    | |
| `fr`   | French     | "chats" → "chat", "mangent" → "mang" |
| `hu`   | Hungarian  | |
| `it`   | Italian    | "gatti" → "gatt" |
| `nb`   | Norwegian  | |
| `nl`   | Dutch      | |
| `pt`   | Portuguese | |
| `ro`   | Romanian   | |
...
For languages not in this list (e.g., Chinese, Japanese, Korean), text is tokenized and lowercased but not stemmed. A warning is logged during index creation.

**Example -- French BM25 index:**

```rust
let config = Bm25CreateConfig::new("french-search", "docs:main", query)
    .with_language("fr");

let result = fluree.create_full_text_index(config).await?;
```

Query text is also analyzed with the French pipeline, so a query for "chats" matches documents containing "chat", "chats", or "chattons".
...
```rust
use fluree_db_query::bm25::{Analyzer, Bm25Scorer, Language};

// Load the index
let index = fluree.load_bm25_index("article-search:main").await?;

// Analyze query terms (use the same language as the index)
